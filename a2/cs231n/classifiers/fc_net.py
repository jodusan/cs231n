...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        loss, a_grads = softmax_loss(scores, y)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        for idx, val in enumerate(hidden_dims):
            sidx = str(idx+1)
            self.params['W'+sidx] = np.random.normal(0, weight_scale, (prev, val))
            self.params['b'+sidx] = np.zeros(val)
            if self.use_batchnorm:
                self.params['gamma'+sidx] = np.ones(val)
                self.params['beta'+sidx] = np.zeros(val)
            prev = val

        # Output layer
        self.params['W'+str(self.num_layers)] = np.random.normal(0, weight_scale, (prev, num_classes))
    ...
    def loss(self, X, y=None):
        ...
        for idx in range(1, self.num_layers):
            ...
            scores, cache_af[sidx] = affine_forward(scores,
                                                    ...
                                                    self.params['b'+sidx])
            if self.use_batchnorm:
                scores, cache_bn[sidx] = batchnorm_forward(scores,
                                                           self.params['gamma'+sidx],
        ...
        # Output layer
        sidx = str(self.num_layers)
        dLayer, grads['W'+sidx], grads['b'+sidx] = affine_backward(a_grads, cache_af[sidx])
        ...
        for idx in reversed(range(1, self.num_layers)):
            ...
            # Loss regularization, useful to go in this loop
            reg_L2 += np.sum(self.params[W] * self.params[W])

        loss += 0.5 * self.reg * reg_L2
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
